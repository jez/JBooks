...
public class Customer implements Comparable<Customer>
{
	 ...
	 *
	 * @return whether or not the transaction was removed successfully
	 */
	public boolean remove(Transaction transaction)
	{
		boolean result = transactions.remove(transaction);

		if(transaction.getType() == TType.PAYMENT)
			totalReceived -= -(transaction.getAmount());
		else if(transaction.getType() == TType.TIP)
			totalReceived -= transaction.getAmount();
		if(transaction.getType() != TType.TIP)
	...
	public String getXML()
	{
		...
		//Adds the name, address, phone number, and email to the result
		result += "<customer>\n";
