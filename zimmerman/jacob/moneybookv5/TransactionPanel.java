...
class TransactionPanel extends JPanel
{
	...
	String[] types = {"Grass cut", "Grass cut half off", "Grass cut with weedwhack", "Water plants", "Spread mulch", "Tip"};
	...
	public TransactionPanel(int type, GregorianCalendar startDate, GregorianCalendar endDate, double moneyReceived, double moneyPending)
	{
		...
		moneyReceivedLabel.addMouseListener(new SwitchAmounts());
		moneyPendingLabel = new JLabel("Money pending:");
		moneyPendingLabel.addMouseListener(new SwitchAmounts());
		...
		moneyReceivedField = new JTextField("25.0");
	...
	//A class that is used to switch the money received and money pending amounts when the respective labels are clicked on
	private class SwitchAmounts extends MouseAdapter
	{
		public void mouseClicked(MouseEvent event)
		{
			String temp = moneyReceivedField.getText();
			
			moneyReceivedField.setText(moneyPendingField.getText());
			moneyPendingField.setText(temp);
		}
	}
